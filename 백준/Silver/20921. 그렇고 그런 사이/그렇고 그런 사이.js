...
- 이 규칙을 이용해서 문제를 풀면 되는데, peoples [1 2 3 4] 에서 맨 뒤부터 차례대로 몇가지의 그렇고 그런사이를 만들 수 있는지 확인한다.
- 이때, 정확히 K개의 그렇고 그런 사이를 만들어야 하기 때문에 그보다 넘치면 안된다.
- 그렇기 때문에 N가 i보다 크거나 같은 경우를 조건으로 둔다.
- N-1부터 1까지 줄어드는 i는 맨 뒤부터 peoples 배열 내 각 원소들이 가질 수 있는 가지 수이다.
(peoples[3] = 4, 4가 가질 수 있는 가지 수는 3가지)
